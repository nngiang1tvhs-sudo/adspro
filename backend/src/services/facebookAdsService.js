  ...
  try {
    ...
    const data = await apiCall(`/${campaignExternalId}/adsets`, decrypted.access_token, {
      fields: [
        ...
        `insights.${insightsTimeParam}{spend,impressions,reach,frequency,clicks,ctr,cpc,cpm,conversions,cost_per_conversion,inline_link_clicks,cost_per_inline_link_click,actions,video_continuous_2_sec_watched_actions,purchase_roas,website_purchase_roas}`,
      ].join(','),
      limit: 200,
    });
  ...
  try {
    ...
    const data = await apiCall(`/${adsetExternalId}/ads`, decrypted.access_token, {
      fields: [
        ...
        `insights.${insightsTimeParam}{spend,impressions,reach,frequency,clicks,ctr,cpc,cpm,conversions,cost_per_conversion,actions,inline_link_clicks,cost_per_inline_link_click,video_continuous_2_sec_watched_actions,purchase_roas,website_purchase_roas}`,
      ].join(','),
      limit: 200,
    });
